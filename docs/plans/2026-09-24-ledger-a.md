...
8. **`--as <session>` names the caller explicitly** (a person in a terminal, or a tool acting for a session). It is
   ...
   *Tightened after the final review:* `--as` does not stop the census from being asked. A process inside a
   listed session may act only under that session's own name; outside one, the event records who really
   called (`caller`: the session, `none: …` or `unknown: …`). The rules a move is checked against (profile and
   posts) are read from trunk, and the event records the trunk revision (`rules`).
